/*
 ...
 ------------------------------------------
  ...
  The core mikamp module player structs. Contains MPLAYER handle and a bunch
  of other primarily-internal-use stuff.
  
  Point of Interest: All those awesome Player_* functions (the mplayer API!)
*/
...
// -------------------------------------------------------------------------------------
// This is a hack structure used for speeding up struct->struct copies in the core of
// the player.  It works in Visual Studio/Watcom/Borland, but is probably unreliable in
// other compilers.
//
...
// -------------------------------------------------------------------------------------
// Voice control information.  There's one of these for every voice alloted to a song.
// All voices are spawned forth from a physical channel within the mod, and that channel
// is represented by the 'master' member.
//
...
// -------------------------------------------------------------------------------------
// The master control struct for each channel in a mod.  If a mod has 32 channels, then
// it has 32 of these.  Note the difference between voices and channels!  A mod can use
// hundreds of voices even if it only has 32 control channels.
//
...
// -------------------------------------------------------------------------------------
// State of the module player. This is held as a struct outside MPLAYER so that we
// can capture instances of the player throughout the song for fast-seeking.
//
typedef struct MP_STATE
{
    ...
    SWORD       sngpos;         // current song position.  This should not
                                // be modified directly.  Use Mikamp_NextPosition,
                                // Mikamp_PrevPosition, and Mikamp_SetPosition.
...
// -------------------------------------------------------------------------------------
// Player handle, created by a call to Player_Create or Player_InitSong.
// 
