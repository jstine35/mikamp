...
// -------------------------------------------------------------------------------------
// Mikamp's custom module player sample structure, used in the place of the MD_SAMPLE,
...
typedef struct UNISAMPLE
{
    ...
    enum SF_BITS     bitdepth; // bitdepth of sampledata on disk
    enum MD_CHANNELS channels; // number of channels (mono/stereo)
    enum SL_COMPRESS compress; // compression status of the sample.
...
typedef struct UNIMOD
{
    ...
    // For further details about there variables, see the Mikamp Docs.
...
{   
    ...
    void      *(*Init)(void);                // creates an instance handle for loading songs.
    void       (*Cleanup)(void *inst);  // clean up (free) instance

    BOOL       (*Load)(void *handle, UNIMOD *of, MMSTREAM *mmfile);
...
MMEXPORT void     Mikamp_RegisterAllLoaders(void);
MMEXPORT MLOADER *Mikamp_LoaderInfo(int loader);
...
MMEXPORT void     ML_RegisterLoader(MLOADER *ldr);  // use the macro Mikamp_RegisterLoader instead
...
// -------------------------
// Various module loaders that come pre-packaged with mikamp and/or have
