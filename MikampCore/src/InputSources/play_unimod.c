...
------------------------------------------
 play_unimod.c
  ...
  all effects.  BIG!  This is the 'core' of mikamp.  The heart of the beast,
  you might even say.
...
static int __inline DoPan(int envpan, int pan)
{
    ...
    // Air's Official Mikamp Panning thingie:
    //  And just what does a Mikamp Panning thingie do?  It modifies the panning
...
MPLAYER *Player_Create(const UNIMOD *mf, uint flags)
{
    ...
    {   MM_ALLOC   *allochandle;
        ...
        _mmalloc_setshutdown(allochandle, (void (*)(void *block))player_shutdown, ps);
...
// _____________________________________________________________________________________
// Turns on Mikamp's smart looping system, and forces the song to loop the sepcific
// number of times.  Note that the behavior of the loopcount varies depending on if PF_LOOP
...
void Player_Register(MD_VOICESET *vs, MPLAYER *ps, uint maxchan)
{
    ...
    Voiceset_SetPlayer( vs, (uint (*)(MD_VOICESET *, void *))Player_HandleTick, ps );
