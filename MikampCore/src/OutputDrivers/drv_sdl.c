...
------------------------------------------
 ...
 Mikamp driver for output via SDL (Simple DirectMedia Layer)

 by Jan L�nnberg (lonnberg@mbnet.fi)

 Notes: SDL's sound handling is based on a callback function that fills the sound
 buffer, while Mikamp seems to use a (more or less) regular interrupt that fills the
...
// =====================================================================================
{
  ...
  if((SDL_Init(SDL_INIT_AUDIO|SDL_INIT_NOPARACHUTE)==-1)) {
    _mmlog("Mikamp > drv_sdl > Failed to initialise SDL.");
    return 1;
  }
...
// =====================================================================================
{
  ...
  /* Mikamp uses unsigned 8 bits or signed 16 bits (system byte order). */
