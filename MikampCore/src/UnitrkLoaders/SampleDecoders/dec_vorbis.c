...
// It's assumed that the module has confirmed our sample type, so no detection code
...
static VORBIS_HANDLE *Vorbis_Init(MMSTREAM *mmfp)
{
    ...
    {
        _mmlog("Mikamp > dec_vorbis > Bad Mojo Error!");
        assert(FALSE);
        return NULL;
    }
    ...
    { 
        _mmlog("Mikamp > dec_vorbis > Error reading first page of Ogg bitstream data.");
        assert(FALSE);
        return NULL;
    }
    ...
    { 
        _mmlog("Mikamp > dec_vorbis > Error reading initial header packet.");
        assert(FALSE);
        return NULL;
    }
...
#endif
    {
        _mmlog("Mikamp > dec_vorbis > This Ogg bitstream does not contain Vorbis audio data.");
        assert(FALSE);
        return NULL;
    }
    ...
    while(i < 2)
    {
        ...
        if(synclen==0 && (i < 2))
        {   _mmlog("Mikamp > dec_vorbis > End of file before finding all Vorbis headers!");
            assert(FALSE);
            return NULL;
        }
