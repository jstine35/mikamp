...
------------------------------------------
 load_it.c
  ...
  ImpulseTracker (IT) module loader.  Created by Jeffrey Lim, who, unfortunately, had a lack of
  knowledge and experience when it comes to making good file-formats.
*/
...
void *IT_Init(void)
{
    ...
    handle = _mmobj_new( NULL, IT_HANDLE );
...
BOOL IT_Load(IT_HANDLE *h, UNIMOD *of, MMSTREAM *mmfile)
{
    ...
    {   ITSAMPLE s;
        ...
        if(s.vibrate)
        {   eq->vibflags |= AV_IT;
            switch(s.vibwave)   // convert IT's auto-vibrato types to Mikamp's
        ...
        if(s.flag & 2)  q->bitdepth = SF_BITS_16;
        if(h->mh.cwt >= 0x200)
        ...
        {   ITINSTHEADER ih;
            ...
            {   BOOL inuse;      // used to determine if the envelope data is actually worth keeping
                ...
                {   
                    ...
                    // of the mikamp cutoff, whic supports a wider range (or I think it does ;)
                    ...
                    //    scale to convert IT's cutoff to mikamp's extended cutoff range!
