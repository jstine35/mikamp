...
------------------------------------------
 load_mtm.c
  ...
  MTM module loader.  Multi-Tracker is actually a continuation of Tran's .669 tracker, by
  StarScream.  It was made more PT-style, and hence a lot easier to support.
...
void *MTM_Init(void)
{
    ...
    retval = _mmobj_new( NULL, MTM_HANDLE );
...
BOOL MTM_Load(MTM_HANDLE *h, UNIMOD *of, MMSTREAM *mmfile)
{
    ...
    if(!(h->globtrack = _mmobj_array( h, h->numtrk = of->numtrk, UBYTE *))) return 0;
