...
------------------------------------------
 load_s3m.c
  ...
  Screamtracker (S3M) module loader.  This one is of Future Crew (dmeo group) fame and
  is the successor to STM, though the two formats have nothing in common.
  
  For the most part this format actually makes SENSE -- except in cases where ImpulseTracker
  and Modplug Tracker decided to implement their own buggy extensions.
*/
...
// _____________________________________________________________________________________
//
BOOL S3M_ReadPattern(UTRK_WRITER *ut, MMSTREAM *mmfile, UBYTE *poslookup, SBYTE *remap, int effectver)
{
    int row=0,flag,ch;
...
BOOL S3M_Load(ST3HANDLE *h, UNIMOD *of, MMSTREAM *mmfile)
{
    ...
    for(t=0; t<of->numsmp; t++)
    {
        ...
        if(s.flags & 4) q->bitdepth  = SF_BITS_16;
