...
------------------------------------------
 load_stm.c
  ...
  (original elite 15-yr old coder comment left in for posterity)
  
  STM is essentially just a slightly customized version of Amiga modules.
*/
...
void *STM_Init(void)
{
    ...
    return _mmobj_allocblock( NULL, STMHEADER );
}
...
{
    uint       t; 
    ULONG      Mikamp_ISA; // We MUST generate our own ISA - NOT stored in the stm
    UNISAMPLE *q;
    ...
    Mikamp_ISA = _mm_ftell(mmfile);
    Mikamp_ISA = (Mikamp_ISA+15) & 0xfffffff0;

    for(t=0; t<of->numsmp; t++)
    {   // load sample info

        q->samplename   = DupStr(of->allochandle, mh->sample[t].filename,12);
        q->seekpos      = Mikamp_ISA;
