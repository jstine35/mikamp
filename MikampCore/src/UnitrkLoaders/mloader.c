...
------------------------------------------
 mloader.c
  ...
  Frontend interface for the dozen or so mikamp loaders.
...
// _____________________________________________________________________________________
//
MLOADER *Mikamp_LoaderInfo(int loader)
{
    int      t;
    MLOADER *l;
...
BOOL AllocSamples(UNIMOD *of, BOOL ext)
{
    ...
    if( ext )
    {
        if( (of->extsamples = _mmobj_array( of->allochandle, of->numsmp, EXTSAMPLE )) == NULL ) return 0;
        of->flags |= UF_EXTSAMPLES;
    }

    if( (of->samples = _mmobj_array( of->allochandle, of->numsmp, UNISAMPLE )) == NULL ) return 0;
    ...
    for( u=0; u<of->numsmp; u++ )
    {
        ...
        of->samples[u].bitdepth   = SF_BITS_8;
